...
public class JogoActivity extends Activity {
	...
	private Jogo carregaVariaveisDoJogador(Intent jogoIntent) {
		...
		Jogo jogo = new Jogo(nomeJogador, nivel, null);
		setJogoAtual(jogo);
		return jogo;
	}
	...
	private void verificaFimDoJogo() {
		if (palavrasRestantes == 0) {
			atualizaVariaveisFimDeJogo("Parab�ns. Fim de jogo!");
		...
		return new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					mostraDialog("Esta n�o � uma palavra listada!",
							alertaListener());
					apresentaBotoesPalavra();

				} catch (PalavraJaEncontradaException pe) {
					mostraDialog("Esta palavra j� foi encontrada!",
							alertaListener());
					apresentaBotoesPalavra();
		...
		return new OnClickListener() {
			...
			public void onClick(View v) {
				if (palavrasRestantes == 0) {
					...
					atualizaVariaveisFimDeJogo("Voc� desistiu. Fim de jogo!");
				}
			}
	...
	private void atualizaPontuacao() {
		pontuacaoTextView.setText("Pontua��o: " + jogoAtual.getPontuacao());
	}
